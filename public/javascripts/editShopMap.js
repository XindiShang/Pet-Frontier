...
// 给出省份默认值
for (var i, j = 0; i = provinceVal.options[j]; j++) {
    if (i.value === shop.province) {
        provinceVal.selectedIndex = j;
        const selectedProvince = provinceVal.value
        for (let province in provinces) {
            if (province === selectedProvince) {
                cityVal.options.length = 0
...
// 给出城市默认值
for (var i, j = 0; i = cityVal.options[j]; j++) {
    if (i.value === shop.city) {
        cityVal.selectedIndex = j;
        break;
    }
}
...
function initGeo() {
    ...
    var infoWindow = new AMap.InfoWindow({
        isCustom: true,  
        content: createInfoWindow(createTitleInit(shop), createContentInit(shop)),
...
function createTitleInit(shop) { 
    var title = shop.name
    return title;
}

function createContentInit(shop) {  
    content = [];
